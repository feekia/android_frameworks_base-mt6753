...
// WLD Start bid 1356:[26606]OTA Hub: OTA package is not getting applied after download 
...
public class RecoverySystem {
    ...
    public static void installPackage(Context context, File packageFile)
		...
		*/
		...
		Log.d(TAG, "installPackage: path:" + filename);
		
		if(ExistSDCard(context)) {
			if(filename.contains("/mnt/sdcard2")) {
				filename = filename.replace("/mnt/sdcard2", "/sdcard2");
			}else if (filename.contains("/mnt/sdcard")) {
				filename = filename.replace("/mnt/sdcard", "/sdcard");
			}else {
				Log.d(TAG, "package path error:" + filename);
			}
		}
		else {
			filename = filename.replace("/mnt/sdcard", "/sdcard2");
		}
		
		Log.d(TAG, "installPackage: parse:" + filename);
	...
	// WLD Start bid 1356:[26606]OTA Hub: OTA package is not getting applied after download 
	private static boolean ExistSDCard(Context context) {  
		StorageManager storageManager = (StorageManager) context.getSystemService(Context.STORAGE_SERVICE);
		
		String sdcard0Status = storageManager.getVolumeState("/storage/sdcard0");
		String sdcard1Status = storageManager.getVolumeState("/storage/sdcard1");
		
		if(sdcard0Status.contains(Environment.MEDIA_MOUNTED) && sdcard1Status.contains(Environment.MEDIA_MOUNTED))
			return true;
		else
			return false;
	}
