...
public class KeyguardSimPinPukMeView extends KeyguardPinBasedInputView {
    ...
    public void resetState(boolean forceReload) {
        ...
        if (count < 2) {
            ...
            if (simState == IccCardConstants.State.PIN_REQUIRED) {
                msg = android.text.TextUtils.isEmpty(displayName)? rez.getString(R.string.kg_sim_pin_instructions): 
					rez.getString(R.string.kg_sim_pin_instructions_multi, displayName);
                mUnlockEnterState = STATE_ENTER_PIN;
            } else if (simState == IccCardConstants.State.PUK_REQUIRED) {
