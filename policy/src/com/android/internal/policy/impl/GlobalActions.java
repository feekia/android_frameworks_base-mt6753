...
class GlobalActions implements DialogInterface.OnDismissListener, DialogInterface.OnClickListener  {
	...
	private static final String GLOBAL_ACTION_KEY_CONNECT = "connect";
	...
	//A hsm data connect
	private ToggleAction.State mDataConnetState = ToggleAction.State.Off;
	private ToggleAction mDataConnetAction;
    ...
    public GlobalActions(Context context, WindowManagerFuncs windowManagerFuncs) {
		...
		filterData.addAction(TelephonyIntents.ACTION_DEFAULT_DATA_SUBSCRIPTION_CHANGED);
		filterData.addAction(WifiManager.NETWORK_STATE_CHANGED_ACTION);
		mContext.registerReceiver(mDateConnectReceiver, filterData);
    }
    ...
    private GlobalActionsDialog createDialog() {
        ...
        mAirplaneModeOn = new ToggleAction(
            ...
            public boolean isEnabled() {
                ...
                return isAirplaneModeAvailable;
            }
        };
        ...
        for (int i = 0; i < defaultActions.length; i++) {
			...
			// [A] hsm  GloatAction  add reboot , data connect 0724
			mItems.add(
			...
			}else if(GLOBAL_ACTION_KEY_CONNECT.equals(actionKey)){
			mItems.add(
				mDataConnetAction = new ToggleAction(
                R.drawable.ic_lock_data_connect_on,
                R.drawable.ic_lock_data_connect_off,
                R.string.data_connection,
                R.string.data_connection_on,
                R.string.data_connection_off) {
			    ...
			    try {
			      ...
			      mTelephonyManager.setDataEnabled(!mTelephonyManager.getDataEnabled());
            ...
            protected void changeStateFromPress(boolean buttonOn) {
             ...
             // mDataConnetState=mTelephonyManager.getDataEnabled()? ToggleAction.State.On : ToggleAction.State.Off;
			 // mDataConnetAction.updateState(mDataConnetState);
    ...
    private static class SilentModeTriStateAction implements Action, View.OnClickListener {
        ...
        public void onClick(View v) {
			...
			if(mAudioManager.getRingerMode()==0){
				Settings.Global.putInt(mContext.getContentResolver(),Settings.Global.ZEN_MODE,0);
				Settings.Global.putInt(mContext.getContentResolver(),Settings.Global.ZEN_MODE,2);
			}
	...
	private void onDataConnectChanged(){
		boolean airplaneModeOn = Settings.Global.getInt(
                mContext.getContentResolver(),
                Settings.Global.AIRPLANE_MODE_ON,
                0) == 1;
		if(airplaneModeOn){
			if(mDataConnetAction!=null)
		    mDataConnetAction.updateState(ToggleAction.State.Off);
			return;
		}
		try{
		mDataConnetState=mTelephonyManager.getDataEnabled()? ToggleAction.State.On : ToggleAction.State.Off;
	...
	private BroadcastReceiver mDateConnectReceiver = new BroadcastReceiver() {
		public void onReceive(Context context, Intent intent) {
		    onDataConnectChanged();
		}
	};
