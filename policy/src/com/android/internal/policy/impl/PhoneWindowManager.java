...
import com.android.internal.policy.impl.hall.HallKeyguardView;
import com.android.internal.policy.impl.hall.HallPhoneCallView;
...
public class PhoneWindowManager implements WindowManagerPolicy {
	...
	//hsm add 0908 hall Msg_
	private static final int MSG_ENABLE_HALL_KEYGUARD = 99;
    private static final int MSG_DISABLE_HALL_KEYGUARD = 100;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_LAUNCH_VOICE_ASSIST_WITH_WAKE_LOCK:
				 ...
				 /* [A] NEW Feature: HALL keyguard UI. hsm. */
				case MSG_ENABLE_HALL_KEYGUARD:
                    showHallKeyguard();
                    break;
                    
                case MSG_DISABLE_HALL_KEYGUARD:
                     hideHallKeyguard();
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        /* [A] NEW Feature: HALL keyguard UI. hsm. */
        if (SUPPORT_HALL_KEYGUARD) {
            filter = new IntentFilter(ACTION_PHONE_CHANGE);
            context.registerReceiver(mHallKeyguardBroadcastReceiver, filter);
        }
        /* END. hsm. */
        ...
        mSystemGestures = new SystemGesturesPointerEventListener(context,
                new SystemGesturesPointerEventListener.Callbacks() {
                    ...
                    public void onThreeFingerSwipeUp() {
                        ...
                        intent.setAction("com.android.launcher3.underground.START_ACTIVITY");
                    ...
                    public void onThreeFingerSwipeDown() {
                        Intent intent = new Intent();
                        if (Settings.Secure.getInt(mContext.getContentResolver(), "three_finger_swipe", 0) == 1) {
                            intent.setAction("com.android.launcher3.underground.START_ACTIVITY");
                            intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
			...
			// Start bid xxxx:when in factory model , let the application handle the key
			if (SystemProperties.get("persist.sys.factory.status").equals("1")) {
				Log.i(TAG, "interceptKeyBeforeDispatching: in facatory mode");
				return 0;
            }
			// End bid xxxx
            if (!down) {
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        // Handle special keys.
        switch (keyCode) {
			/* [Add] NEW Feature: HALL hsm 20150828. start */	
        	//hsm add show hall 
        	case KeyEvent.KEYCODE_F9:{
				if (SystemProperties.get("persist.sys.factory.status").equals("1")) {
					result &= ACTION_PASS_TO_USER;
		        		...
		        		result &= ~ACTION_PASS_TO_USER;
		        		if(down){
		             		if (getSupportHallKeyguard()) {
		                        switchHallKeyguard(false);
		                    }else{
		                      //close
   ...
   ///@}
    ...
    private void finishScreenTurningOn() {
        ...
        synchronized (mLock) {
			...
			if(mHallKeyguardSecure){
				mKeyguardDelegate.dismiss();
				mHallKeyguardSecure=false;
			}
        }
	...
	/* [Add] NEW Feature: HALL keyguard UI. hsm 20150917. start */	
	static final boolean SUPPORT_HALL_KEYGUARD = true;
	static final String PROP_HALL_KEY = "persist.sys.hall_status";
	static final String ACTION_HALL_KEY = "com.malata.action.HALL_KEY";
    static final String ACTION_HALL_VIEW = "com.malata.action.HALL_VIEW";
    static final String ACTION_PHONE_CHANGE = "com.malata.phone.change";
    static final String TAG_INCOMING = "incoming";
    static final String FILE_NODE = "/sys/devices/platform/mtk-kpd/input/input0/status";
    static final String TP_GESTURE_FILE_NODE = "/sys/bus/i2c/devices/0-0038/status_d0";
    static final String BG_TOUCH_FILE_NODE = "/sys/devices/platform/aw9136/switch";

	HallKeyguardView mHallKeyguardView = null;
	HallPhoneCallView mHallPhoneCallView = null;
	static final boolean isHallSupport=true;
	TelephonyManager mTelephonyManager=null;
	SubscriptionManager mSubscriptionManager=null;
	ArrayList<HallPhoneStateListener> mPhoneStateList=new ArrayList<HallPhoneStateListener>();
	HashMap<Integer,Boolean> mSubIdleMap=new HashMap<Integer,Boolean>();
	int mSystemSleepOffTimeOut=30000;
	boolean mHallKeyguardSecure=false;


	
	private boolean getSupportHallKeyguard(){
	     return Settings.System.getInt(mContext.getContentResolver(),"smart_flip_cover",1)==1?true : false;
	}

	final void switchHallKeyguard(boolean open) {
        Log.d(TAG, "switchHallKeyguard: open=" + open + ", mCallStatus=" + mCallStatus);
        switchSystemKeyguard(open);
        
        if (!open && !mCallStatus) {
            /*[A] remove disable message before enable. */
            if(mHandler.hasMessages(MSG_DISABLE_HALL_KEYGUARD)){
                mHandler.removeMessages(MSG_DISABLE_HALL_KEYGUARD);
            }
            /*End  */
            mHandler.sendEmptyMessage(MSG_ENABLE_HALL_KEYGUARD);
        } else {
            mHandler.removeMessages(MSG_DISABLE_HALL_KEYGUARD);
            if (isKeyguardSecure()) {
                mHandler.sendEmptyMessage(MSG_DISABLE_HALL_KEYGUARD);
            } else {
                mHandler.sendEmptyMessageDelayed(MSG_DISABLE_HALL_KEYGUARD, 750);
            }
        }
        
        // status chaned notification
        final Intent intent = new Intent(ACTION_HALL_KEY);
        SystemProperties.set(PROP_HALL_KEY, open ? "1" : "0");
        intent.putExtra("open", open);
        mContext.sendBroadcast(intent);
    }

	final void switchSystemKeyguard(boolean open) {
        if (open) {

			// regain system sleep mode
            Settings.System.putInt(mContext.getContentResolver(),"screen_off_timeout",mSystemSleepOffTimeOut);
			
			// is Support auto unlock--> Secure
			if(Settings.System.getInt(mContext.getContentResolver(),"smart_flip_auto_unlock", 1)!=1){
				return;
			}
            if (mKeyguardDelegate != null &&!mKeyguardDelegate.isSecure()) {
               ...
               if(pm.isScreenOn()){
			      ...
			      mHallKeyguardSecure=true;
			    }
            }
        } else {
            
            lockNow(null);
		    //  nonsupport hall view -> go to sleep
			if(!isHallSupport){
				PowerManager pm = (PowerManager) mContext.getSystemService(Context.POWER_SERVICE);  
                pm.goToSleep(SystemClock.uptimeMillis());
				return;
			...
			//set hall sleep mode 
			//setp 1: get system sleep off timeout
			mSystemSleepOffTimeOut=Settings.System.getInt(mContext.getContentResolver(),"screen_off_timeout",30000);
            // setp 2: set hall sleep off timeout
			String sleepMode=Settings.System.getString(mContext.getContentResolver(),
				"smart_flip_sleep_mode");
			if (sleepMode == null || "".equals(sleepMode)) {
				return;
		    }else if("0".equals(sleepMode)){
		    	PowerManager pm = (PowerManager) mContext.getSystemService(Context.POWER_SERVICE);  
                pm.goToSleep(SystemClock.uptimeMillis());
		    }else if("-1".equals(sleepMode)){
		    } else {
		        int sleepTime=30000;
		        try{
					sleepTime=Integer.valueOf(sleepMode);
				}catch(Exception e){
				   Log.w(TAG,"at switchSystemKeyguard "+e);
				}
		        Settings.System.putInt(mContext.getContentResolver(),"screen_off_timeout",sleepTime);
		    }
        }
    }

	private void hidePhoneCallView(){
		if(isHallOpen()){
			mHallPhoneCallView.setVisibility(View.GONE);
		    mHallKeyguardView.setVisibility(View.GONE);
			return;
		}
		Iterator iter = mSubIdleMap.entrySet().iterator();
        while(iter.hasNext()) {
		...
		mHallPhoneCallView.setVisibility(View.GONE);
		mHallKeyguardView.setVisibility(View.VISIBLE);
	}
	...
	private void showPhoneCallView(){
		...
		mHallPhoneCallView.setVisibility(View.VISIBLE);
		mHallKeyguardView.setVisibility(View.GONE);
	}

	private class HallPhoneStateListener extends PhoneStateListener{
			private int subscription;

			HallPhoneStateListener(int subscription){
				super(subscription);
			...
			public void onCallStateChanged(int state, String number) {
				mHallPhoneCallView.setOperator(mTelephonyManager.getNetworkOperatorName());
				switch (state) {
				// IDLE state
				case TelephonyManager.CALL_STATE_IDLE:
					mSubIdleMap.put(subscription,false);
					hidePhoneCallView();	  
					Log.i(TAG,"--PhoneStateListener--CALL_STATE_IDLE");
					break;
				// OFFHOOK state
				case TelephonyManager.CALL_STATE_OFFHOOK:
					mSubIdleMap.put(subscription,true);
					Log.i(TAG,"--PhoneStateListener--CALL_STATE_OFFHOOK--"+subscription);
					int out=0;
					try{
					    out=Settings.System.getInt(mContext.getContentResolver(), "hall_outgoing");
						
					}catch(android.provider.Settings.SettingNotFoundException e){
						Log.e(TAG, "android.provider.Settings.SettingNotFoundException");
					}
					Log.i(TAG,"--PhoneStateListener--CALL_STATE_OFFHOOK>>>out=="+out);
					if(out==0) {
						mHallPhoneCallView.setHintText(mContext
								.getResources().getString(R.string.hall_ongoing_call));
						mHallPhoneCallView.setIncomingLayouVisible(false);
					}else{
					    mHallPhoneCallView.setIncoming(false);
						mHallPhoneCallView.setStateHint();
					}
					showPhoneCallView();
					break;
				// RINGING state
				case TelephonyManager.CALL_STATE_RINGING:
					mSubIdleMap.put(subscription,true);
					mHallPhoneCallView.setIncoming(true);
					Settings.System.putInt(mContext.getContentResolver(), "hall_outgoing",0);
					showPhoneCallView();
					mHallPhoneCallView.setHintText(mContext
							.getResources().getString(
									R.string.hall_incoming_call));
					mHallPhoneCallView.setPhoneNumer(number);
					break;
				default:
					break;
				}
	...
	private void initPhoneStateListener(){
		...
		mPhoneStateList.clear();
		mSubIdleMap.clear();
		if(null != subInfoList && !subInfoList.isEmpty()){
			HallPhoneStateListener listener=null;
			for (SubscriptionInfo record : subInfoList) {
				int subId = record.getSubscriptionId();
				Log.d(TAG,"subId---"+subId);
				listener=new HallPhoneStateListener(subId);
				mPhoneStateList.add(listener);
				mSubIdleMap.put(subId,false);
			}
		}
	}
    final void createHallKeyguard() {
        if (mHallKeyguardView == null) {
            Log.d(TAG, "createHallKeyguard");
			mHallKeyguardView = new HallKeyguardView(mContext);
			mHallPhoneCallView = new HallPhoneCallView(mContext);
            ...
            final WindowManager.LayoutParams lp = new WindowManager.LayoutParams(
                    WindowManager.LayoutParams.MATCH_PARENT,
                    WindowManager.LayoutParams.MATCH_PARENT);
			lp.type = WindowManager.LayoutParams.TYPE_HALL_KEYGUARD;
            ...
            lp.setTitle("HallKeyguard");
            //mHallKeyguardView.setVisibility(View.GONE);
            WindowManager wm = (WindowManager)mContext.getSystemService(Context.WINDOW_SERVICE);
            wm.addView(mHallKeyguardView, lp);
			wm.addView(mHallPhoneCallView, lp);

            boolean open = false;
            if (isHallCloseByDriver() && !isAlarmBoot()) {
                showHallKeyguard();
            }
            else {
                hideHallKeyguard();
            ...
            try {
               ...
               Log.e(TAG, "createHallKeyguard: " + e);
            }
        }
    }

    final void showHallKeyguard() {
		
        if (mHallKeyguardView != null) {
            notifyHallView(false);
            Log.d(TAG, "showHallKeyguard");
			mHallKeyguardView.setVisibility(View.GONE);
			mHallPhoneCallView.setVisibility(View.GONE);
			if(isHallSupport){
				mHallKeyguardView.setVisibility(View.VISIBLE);
            ...
            mHallKeyguardView.showKeyguard();            
        }
    }

    final void hideHallKeyguard() {
		
        if (mHallKeyguardView != null) {
            notifyHallView(true);
            Log.d(TAG, "hideHallKeyguard");
            mHallKeyguardView.hideKeyguard();
	...
	private void registerPhoneStateListener(boolean isRegister){
		if(!isHallSupport){
			return;
		}
		if(isRegister){
			initPhoneStateListener();
			for(HallPhoneStateListener listener : mPhoneStateList){
    ...
    void notifyHallView(boolean open) {
        final Intent intent = new Intent(ACTION_HALL_VIEW);
        intent.putExtra("open", open);
        try {
            mContext.sendBroadcast(intent);
        } catch (Exception e) {
           Log.e(TAG, "notifyHallView: " + e);
        }
    }

    static final boolean isHallOpen() {
        return "1".equals(SystemProperties.get(PROP_HALL_KEY, "1"));
    }

    static final boolean isHallCloseByDriver() {
        FileInputStream fis = null;
        try {
            fis = new FileInputStream(FILE_NODE);
            int status = fis.read();
            return status == 0;
        } catch (Exception e) {
            Log.e(TAG, "isHallCloseByDriver: " + e);
        } finally {
            if (fis != null) {
                try {
                    fis.close();
                } catch (Exception e1) {
                    Log.e(TAG, "isHallCloseByDriver: " + e1);
                }
            }
        }
    ...
    static final void writeTpGestureFile(String value) {
        writeFile(TP_GESTURE_FILE_NODE, value);
    }

    static final void writeBgTouchFile(String value) {
        writeFile(BG_TOUCH_FILE_NODE, value);
    }

    static final void writeFile(final String path, String value) {
    ...
    boolean mCallStatus;
    final BroadcastReceiver mHallKeyguardBroadcastReceiver = new BroadcastReceiver() {
        @Override public void onReceive(Context context, Intent intent) {
            final String action = intent.getAction();
            Log.d(TAG, "onReceive: action=" + action);
            if (ACTION_PHONE_CHANGE.equals(action)) {
                final boolean hasIncommingPhone = intent.getIntExtra(TAG_INCOMING, 0) == 1;
                Log.d(TAG, "onReceive: hasIncommingPhone=" + hasIncommingPhone);
                if (!isHallOpen() && !hasIncommingPhone) {
                    mHandler.sendEmptyMessage(MSG_ENABLE_HALL_KEYGUARD);
                } else {
                    mHandler.sendEmptyMessage(MSG_DISABLE_HALL_KEYGUARD);
                }
                mCallStatus = hasIncommingPhone;
            }
        }
    };
   ...
   /* [End]  NEW Feature: HALL keyguard UI. hsm  */
