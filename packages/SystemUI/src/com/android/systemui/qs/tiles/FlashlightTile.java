...
public class FlashlightTile extends QSTile<QSTile.BooleanState> implements
    ...
    protected void handleClick() {
        ...
        if(newState&&mBatteryLevel<=15){
        	mContext.sendBroadcast(new Intent(Intent.ACTION_CLOSE_SYSTEM_DIALOGS));
        	Toast.makeText(mContext, R.string.battery_low_title_flash, Toast.LENGTH_SHORT).show();
        	return;
        }

        Settings.System.putInt(mContext.getContentResolver(), "flash_light_state", newState ? 1:0 );
        mFlashlightController.setFlashlight(newState);
        refreshState(newState ? UserBoolean.USER_TRUE : UserBoolean.USER_FALSE);
		mContext.sendBroadcast(new Intent("com.android.flashlight.STATE_CHANGE"));
    }
